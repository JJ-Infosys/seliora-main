...
export default function AdminSettingsPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-amber-600" /></div>;
  }
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Manage admin credentials, contact info, and site content</p>
      </div>

      {/* ── Admin Account ── */}
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
        <UserCircleIcon className="h-10 w-10 text-amber-600 flex-shrink-0" />
        <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-6 space-y-5">
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          <UserCircleIcon className="h-5 w-5 text-amber-600" /> Account Info
        </h2>
        <div>
        ...
        <hr />
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          <LockClosedIcon className="h-5 w-5 text-amber-600" /> Change Password
        </h2>
        <p className="text-xs text-gray-500 -mt-3">Leave blank to keep the current password</p>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">New Password</label>
          <input type="password" value={form.newPassword} onChange={e => setForm(f => ({ ...f, newPassword: e.target.value }))}
            placeholder="Min 6 characters" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" />
        </div>
        <div>
      ...
      {/* ── Contact Information ── */}
      {contactMessage && (
        <div className={`p-4 rounded-xl text-sm ${contactMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
          {contactMessage.text}
        </div>
      )}

      <form onSubmit={handleContactSave} className="bg-white rounded-xl shadow-md p-6 space-y-5">
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          <PhoneIcon className="h-5 w-5 text-amber-600" /> Contact Information
        </h2>
        <p className="text-xs text-gray-500 -mt-3">Displayed on the public Contact Us page</p>
      ...
      {/* ── Categories ── */}
      {catsMessage && (
        <div className={`p-4 rounded-xl text-sm ${catsMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
          {catsMessage.text}
        </div>
      )}
      <div className="bg-white rounded-xl shadow-md p-6 space-y-4">
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          <TagIcon className="h-5 w-5 text-amber-600" /> Manage Categories
        </h2>
        <p className="text-xs text-gray-500 -mt-2">These categories appear in product forms and the shop filter</p>

        <div className="flex flex-wrap gap-2">
          {categories.map(cat => (
            <span key={cat} className="flex items-center gap-1 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-800 rounded-full text-sm">
              {cat}
              <button onClick={() => removeCategory(cat)} className="ml-1 text-amber-500 hover:text-red-500 font-bold leading-none">×</button>
            </span>
          ))}
        </div>
      ...
      {/* ── Terms & Conditions ── */}
      {termsMessage && (
        <div className={`p-4 rounded-xl text-sm ${termsMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
          {termsMessage.text}
        </div>
      )}

      <form onSubmit={handleTermsSave} className="bg-white rounded-xl shadow-md p-6 space-y-5">
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          <DocumentTextIcon className="h-5 w-5 text-amber-600" /> Terms & Conditions
        </h2>
        <p className="text-xs text-gray-500 -mt-3">Displayed on the public Terms & Conditions page</p>
